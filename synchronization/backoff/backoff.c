...
/**
 * ******************************************************
 *  For deadlock run with ./backoff 0
 *  To see backoffs happening run with sched_yield: ./backoff 1 1
 *  Or with sleep: ./backoff 1 -1
 *
 *  For more in depth analysis, try compiling with -DDEBUG
 *  and running fewer iterations.
 * ******************************************************
**/
    ...
    for (iterate = 0; iterate < ITERATIONS; iterate++) {
        ...
        for (i = 0; i < 3; i++) {
            if (i == 0) {
                ...
                if (status != 0) err_abort (status, "First lock");
            } else {
                if (backoff)     status = pthread_mutex_trylock (&mutex[i]);
                else             status = pthread_mutex_lock (&mutex[i]);
...
int main (int argc, char *argv[])
{
    ...
    if (status != 0) err_abort (status, "Create backward");
