...
typedef struct crew_tag {
    ...
    long    path_max;                   /* Filesystem path limit */
    long    name_max;                   /* Filesystem name limit */
} crew_t, *crew_p;
...
void *worker_routine (void *arg)
{
    ...
    entry = (struct dirent*)malloc (
        sizeof (struct dirent) + crew->name_max);
    if (entry == NULL)
    ...
    while (1) {
        ...
        while (1) {
            ...
            new_work->path = (char*)malloc (crew->path_max);
